...
function DetailWorker({icon,data,heading,title}){
    const [modalvisible,setVisible]=useState(false)
    return(
        <>
        <View>
         <TouchableOpacity onPress={()=>setVisible(true)}>
            <View className='bg-gray-100 border border-orange-500 p-4 rounded-md my-5'>
            ...
            <View className='p-2 flex-1 w-[60%] my-4 rounded-lg bg-purple-300'>
             ...
             <Payment modalvisible={modalvisible} setVisible={setVisible} title={'Payment'}/> 
        </>
    );
}
